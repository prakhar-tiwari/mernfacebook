  ...
  return (
    <div key={post._id} className={classes.postDescription}>
      <Typography component="div" style={{ textAlign: "left" }}>
        ...
        {tags.length === 1 ? (
          <p>
            - with <a href="#">{tags[0].user.name}</a>
          </p>
        ) : tags.length > 1 ? (
          <p>
            - with <a href="#">{tags[0].user.name}</a> and{" "}
            <a onClick={handleTagPopper}>{tagsCount} others</a>
          </p>
        ) : null}
        <Popper
        ...
        >
          <div className={classes.popperContent}>
            ...
            {otherTags.map((oTag) => (
              <Typography key={oTag.user._id} className={classes.typography}>
                {" "}
                {oTag.user.name}{" "}
              </Typography>
            ))}
            ...
            ? imageUrls.map((image) => (
                ...
                >
                  <img
                    ...
                    onClick={() => onPhotoClick(post, image._id)}
                    src={
                      image.imageUrl ? `${image.imageUrl}` : "images/404.png"
                    }
                  />
                </GridListTile>
              ))
            ...
            ? videoUrls.map((video) => (
                ...
                >
                  <video
                    ...
                    onClick={() => onVideoClick(post, video._id)}
                    src={
                      video.videoUrl ? `${video.videoUrl}` : "images/404.png"
                    }
                    poster = {video.thumbnailPath ? `${video.thumbnailPath}` :''}
                  />
                </GridListTile>
              ))
            : null}
